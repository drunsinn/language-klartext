...
let current_line_numer = 0;
  ...
  function remove_line_numbers() {
    ...
    if(editor.getGrammar().scopeName == "source.klartext") {
      ...
      const search_regex = new RegExp('^\\/?\\d+\\s{1,2}', 'gm')
      editor.getBuffer().scan(search_regex, remove_line_numbers_iterator)
    } else if (editor.getGrammar().scopeName == "source.g-code") {
      console.debug("Grammar OK - G-Code")
      const search_regex = new RegExp('^N\\s*\\d+\\s+', 'gm')
      editor.getBuffer().scan(search_regex, remove_line_numbers_iterator)
    } else {
  ...
  function add_line_numbers(remove_numbers = true) {
    ...
    const editor = atom.workspace.getActiveTextEditor()
    if (editor.getGrammar().scopeName == "source.klartext") {
      var search_regex = new RegExp('^[\\w;].*?', 'gm')
      editor.getBuffer().scan(search_regex, add_line_numbers_iterator)
    } else {
      console.debug("Inkorrekt Grammar")
  ...
  function correct_number_format(){
    ...
    if(editor.getGrammar().scopeName == "source.klartext") {
      ...
      var search_regex = new RegExp('([\\+-]\\d+,\\d+)', 'gm')
      editor.getBuffer().scan(search_regex, correct_number_format_iterator)
    } else {
      console.debug("Inkorrekt Grammar")
      dialog.showMessageBox({ message: "Function not availible for this file format", buttons: ["OK"] })
    }
...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace',
                                             {'language-klartext:remove_line_numbers': () => remove_line_numbers()}));
    this.subscriptions.add(atom.commands.add('atom-workspace',
                                             {'language-klartext:add_line_numbers': () => add_line_numbers(remove_numbers = false)}));
